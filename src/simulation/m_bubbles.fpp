...
!$acc routine seq
        ...
        if (polytropic) then
            f_cpbw = (0.5*Ca + 2.d0/Web/fR0)*((fR0/fR)**(3.d0*gam)) - 0.5*Ca - 4.d0*Re_inv*fV/fR - 2.d0/(fR*Web)    ! Modified line
        else
...
!$acc routine seq
        ...
        if (polytropic) then
            ...
            tmp1 = -3.d0*gam*(0.5*Ca + 2d0/Web/fR0)*tmp1*fV/fR      ! Modified line
        else
            tmp1 = fpbdot
        end if
...
!$acc routine seq
        ...
        if (polytropic) then
            f_cpbw_KM = 0.5*Ca*((fR0/fR)**(3.d0*gam)) - 0.5*Ca + 1d0    ! Modified line
...
!$acc routine seq
        ...
        if (polytropic) then
            cdot_star = -3d0*gam*0.5*Ca*((fR0/fR)**(3d0*gam))*fV/fR ! Modified line
